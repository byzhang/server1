...
void FileTransferServiceImpl::CloseChannel(
...
void FileTransferServiceImpl::Register(
  ...
  boost::shared_ptr<FileTransferClient> client;
  {
    ...
    if (it == transfer_client_table_.end()) {
      client.reset(FileTransferClient::Create(
          host, "", src_filename, dest_filename, 0));
      client->set_threadpool(&threadpool_);
      init = true;
      transfer_client_table_.insert(make_pair(unique_identify, make_pair(1, client)));
    } else {
      boost::shared_ptr<FileTransferClient> local_client = it->second.second;
    ...
    channel->close_signal()->connect(boost::bind(
        &FileTransferServiceImpl::CloseChannel, this, channel));
  }
  if (init) {
    client->Start();
  }
  client->PushChannel(channel);
}
